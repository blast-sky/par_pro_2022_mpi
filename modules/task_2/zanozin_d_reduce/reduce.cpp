...
#include "../../../modules/task_2/zanozin_d_reduce/reduce.h"
...
void Reduce(int* sendbuf, int* recvbuf, int count, MPI_Datatype type, MPI_Op op,
            int root, MPI_Comm comm) {
  if (ProcRank != root) {
    for (int i = 1; i < ProcSize; i++) {
      if (i == ProcRank)
        MPI_Send(sendbuf, count, type, root, 0, MPI_COMM_WORLD);
    ...
    switch (op) {
      case MPI_MAX: {
        for (int i = 0; i < count; i++) {
          recvbuf[i] =
              *std::max_element(all_elements.begin() + i * ProcSize,
      ...
      case MPI_MIN: {
        for (int i = 0; i < count; i++) {
          recvbuf[i] =
              *std::min_element(all_elements.begin() + i * ProcSize,
                                all_elements.begin() + ((i + 1) * ProcSize));
        }
        break;
      }
      case MPI_SUM: {
        for (int i = 0; i < count; i++) {
          recvbuf[i] =
              std::accumulate(all_elements.begin() + i * ProcSize,
